...
export default function SearchContentWrapper({ make, model }: SearchContentWrapperProps) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();

  // Search parameters state synced with URL, pre-locked to route params
  ...
  // Fetch listings (pre-filtered by locked make on API endpoint level)
  const { data: listings = [], isLoading } = useQuery<MarketplaceListing[]>({
    queryKey: ['programmaticSearch', make, model, searchQuery, selectedDistrict, selectedCondition, selectedFuelType, priceMaxInput, mileageMaxInput, selectedDealRatings.join(','), sortOption],
    queryFn: async () => {
      ...
      try {
        ...
        console.warn('Using mock listings programmatic search fallback:', err);
        return MOCK_LISTINGS;
      }
    }
  });

  // Client side filtering & sorting (locked to path make/model)
  const filteredAndSortedListings = useMemo(() => {
    ...
    return result;
  }, [listings, make, model, searchQuery, selectedDistrict, selectedFuelType, priceMaxInput, mileageMaxInput, selectedDealRatings, sortOption]);

  // Facet count calculation locked to make/model subset
  const facets = useMemo(() => {
    ...
    return { districtCounts, ratingCounts };
  }, [listings, make, model]);
  ...
  const renderFilters = () => (
    <div className="space-y-6">
      {/* Locked Make & Model View */}
      <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 space-y-2">
        <div>
          <span className="block text-[10px] font-bold text-textSecondary uppercase tracking-wider">Locked Make</span>
          <span className="text-sm font-bold text-textPrimary">{make}</span>
        </div>
        <div>
          <span className="block text-[10px] font-bold text-textSecondary uppercase tracking-wider">Locked Model</span>
          <span className="text-sm font-bold text-textPrimary">{model}</span>
        </div>
      </div>

      {/* Search Input */}
      <div>
        <h4 className="text-sm font-bold text-textPrimary mb-2">এই ক্যাটাগরিতে খুঁজুন</h4>
        <div className="relative">
          <input
            type="text"
            placeholder="পছন্দের স্পেকস..."
            value={searchQuery}
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
            <div>
              <span className="text-base font-bold text-textPrimary">
                {convertEnglishToBangla(filteredAndSortedListings.length)}টি {make} {model} গাড়ি পাওয়া গেছে
              </span>
            </div>
          ...
          {isLoading ? (
            ...
            <div className="bg-white p-12 rounded-xl border border-gray-200 shadow-sm text-center max-w-md mx-auto space-y-4">
              ...
              <p className="text-textSecondary text-sm">
                আপনার নির্বাচিত ফিল্টারের সাথে মিলে এমন কোনো {make} {model} গাড়ি পাওয়া যায়নি।
              </p>
              <button 
                onClick={clearAllFilters}
