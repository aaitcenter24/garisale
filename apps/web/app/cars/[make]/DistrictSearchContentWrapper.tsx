...
interface DistrictSearchContentWrapperProps {
  district: string;
}
...
export default function DistrictSearchContentWrapper({ district }: DistrictSearchContentWrapperProps) {
  ...
  // Fetch listings pre-filtered by locked district on API level
  const { data: listings = [], isLoading } = useQuery<MarketplaceListing[]>({
    queryKey: ['districtSearch', district, searchQuery, selectedMake, selectedModel, priceMaxInput, mileageMaxInput, selectedDealRatings.join(','), sortOption],
    queryFn: async () => {
      ...
      try {
        ...
        console.warn('Using mock listings district search fallback:', err);
        return MOCK_LISTINGS;
      }
    }
  });

  // Client side filtering & sorting (locked to path district)
  const filteredAndSortedListings = useMemo(() => {
    let result = listings.filter(item => item.district.toLowerCase() === district.toLowerCase());
    ...
    return result;
  }, [listings, district, searchQuery, selectedMake, selectedModel, priceMaxInput, mileageMaxInput, selectedDealRatings, sortOption]);
  ...
  const facets = useMemo(() => {
    ...
    return { makeCounts, ratingCounts };
  }, [listings, district]);
  ...
  const renderFilters = () => (
    <div className="space-y-6">
      ...
      <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
        <span className="block text-[10px] font-bold text-textSecondary uppercase tracking-wider">Locked Location</span>
        <span className="text-sm font-bold text-textPrimary">{district}</span>
      </div>
      ...
      <div>
        ...
        <div className="relative">
          <input
            type="text"
            placeholder="গাড়ির ব্র্যান্ড, মডেল..."
            value={searchQuery}
          ...
          {isLoading ? (
            ...
            <div className="bg-white p-12 rounded-xl border border-gray-200 shadow-sm text-center max-w-md mx-auto space-y-4">
              ...
              <p className="text-textSecondary text-sm">
                আপনার নির্বাচিত ফিল্টারের সাথে মিলে এমন কোনো গাড়ি {district}-এ পাওয়া যায়নি।
              </p>
              <button 
                onClick={clearAllFilters}
